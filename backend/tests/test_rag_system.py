...
class TestRAGSystem(unittest.TestCase):
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_query_processing_with_tool_usage(self, mock_session_mgr, mock_ai_gen, mock_vector_store, mock_doc_proc):
        """Test end-to-end query processing with tool usage"""
        # Setup mocks
        mock_ai_generator = Mock()
        mock_ai_gen.return_value = mock_ai_generator
        mock_ai_generator.generate_response.return_value = "AI response with tool results"
        
        mock_tool_manager = Mock()
        mock_tool_manager.get_last_sources.return_value = [{"text": "Test Course - Lesson 1", "link": None}]
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_query_processing_with_session_history(self, mock_session_mgr, mock_ai_gen, mock_vector_store, mock_doc_proc):
        """Test query processing with conversation history"""
        # Setup mocks
        mock_ai_generator = Mock()
        mock_ai_gen.return_value = mock_ai_generator
        mock_ai_generator.generate_response.return_value = "Response with history context"
        ...
        mock_ai_generator.generate_response.assert_called_once()
        call_args = mock_ai_generator.generate_response.call_args
        self.assertEqual(call_args[1]["conversation_history"], "Previous conversation context")
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_query_processing_max_results_zero_bug(self, mock_session_mgr, mock_ai_gen, mock_vector_store, mock_doc_proc):
        ...
        mock_ai_generator = Mock()
        mock_ai_gen.return_value = mock_ai_generator
        mock_ai_generator.generate_response.return_value = "I couldn't find any relevant information"
        ...
        def mock_generate_response(query, conversation_history=None, tools=None, tool_manager=None):
            if tools and tool_manager:
                ...
                tool_result = tool_manager.execute_tool("search_course_content", query="MCP basics")
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_add_course_document(self, mock_session_mgr, mock_ai_gen, mock_vector_store, mock_doc_proc):
        ...
        mock_document_processor.process_course_document.assert_called_once_with("test_file.pdf")
    ...
    def test_add_course_folder(self, mock_isfile, mock_listdir, mock_exists, mock_session_mgr, 
                              mock_ai_gen, mock_vector_store, mock_doc_proc):
        ...
        mock_chunks1 = [CourseChunk(content="Content 1", course_title="Course 1", chunk_index=0)]
        mock_chunks2 = [CourseChunk(content="Content 2", course_title="Course 2", chunk_index=0)]
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_get_course_analytics(self, mock_session_mgr, mock_ai_gen, mock_vector_store, mock_doc_proc):
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_error_handling_in_query(self, mock_session_mgr, mock_ai_gen, mock_vector_store, mock_doc_proc):
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_sources_reset_after_query(self, mock_session_mgr, mock_ai_gen, mock_vector_store, mock_doc_proc):
